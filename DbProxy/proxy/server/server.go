...
func parseNode(cfg config.NodeConfig) (*backend.Node, error) {
	...
	if err != nil {
		golog.Error("server", "parseNode", fmt.Sprintf("ParseMaster error:%s,NodeName=%s", err.Error(),cfg.Name), 0)
		return  nil,err
	}

	err = n.ParseSlave(cfg.Slave)
	if err != nil {
		golog.Error("server", "parseNode", fmt.Sprintf("ParseSlave error:%s,NodeName=%s", err.Error(),cfg.Name), 0)
		return  nil,err
	}
	...
	for _, schemaCfg := range schemaCfgList {
		...
		for _, n := range schemaCfg.Nodes {
			...
			if allNodes[n] == nil {
				...
				golog.Error("server", "parseSchemaList", fmt.Sprintf("schema node [%s] config is not exists", n), 0)
		...
		schemas[schemaCfg.User] = &Schema{
			nodes: nodes,
			rule:  rule,
			allow_dbs:schemaCfg.AllowDbList,
		}
...
func (s *Server) UpdateConfig(newCfg *config.Config) {
	...
	if s.configDataVer==newCfg.ConfigVer {
		golog.Info("Server", "UpdateConfig", "config no need update", 0)
		return
	}

	s.configDataVer=newCfg.ConfigVer
