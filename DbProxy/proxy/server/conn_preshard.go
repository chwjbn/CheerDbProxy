...
func (c *ClientConn) preHandleShard(sql string) (bool, error) {
	...
	if executeDB.ExecNode==nil{
		err=mysql.NewError(mysql.ER_ACCESS_DENIED_ERROR,"The backend node is invalid!")
		return false, err
	}
	...
	sqlCheck:=strings.ReplaceAll(sql," ","")
	sqlCheck=strings.ToUpper(sqlCheck)

	//显示数据库列表
	if strings.Index(sqlCheck,"SHOWDATABASES")==0 {

		bNeedFilter:=true

		for _,db:=range c.schema.allow_dbs{
			if db=="*" {
				bNeedFilter=false
			}
		}
		...
		if bNeedFilter{
			...
			rs[0].Values=rs[0].Values[0:0]
			rs[0].RowDatas=rs[0].RowDatas[0:0]

			for _,db:=range c.schema.allow_dbs{
				rs[0].Values = append(rs[0].Values, []interface{}{db})
			}


			//构建rs结果集
			for _, vs := range rs[0].Values {
				var row []byte
				for _, value := range vs {
					b:=hack.Slice(value.(string))
					row = mysql.AppendLenEncStringBytes(row, b)
				}
				rs[0].RowDatas=append(rs[0].RowDatas, row)
			}
