...
func (c *ClientConn) readHandshakeResponse() error {
	...
	if _, ok := c.proxy.users[c.user]; !ok {
		...
		return mysql.NewError(mysql.ER_ACCESS_DENIED_ERROR,fmt.Sprintf("Your account [%s] has not been opened or expired!",c.user))
	}
	...
	if !bytes.Equal(auth, checkAuth) {
		...
		return mysql.NewError(mysql.ER_ACCESS_DENIED_ERROR,"The password you used is incorrect!")
	}
